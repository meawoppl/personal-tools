...
TMP_DIR=/home/meawoppl/build/atl-lap
...
rm -rvf /usr/lib{,64}/liblapack.*
rm -rvf /usr/lib{,64}/libblas.*
rm -rvf /usr/lib/*blas.{a,so}
rm -rvf /usr/lib64/*blas.{a,so}
rm -rvf /usr/lib/*atlas.{a,so}
rm -rvf /usr/lib64/*atlas.{a,so}
rm -rvf /usr/lib/*lapack.{a,so}
rm -rvf /usr/lib64/*lapack.{a,so}

read -p "Press any key to continue..."

# ------------------------------------------------------------------------------
echo -e "[ ${RED} Download lapack-3.3.1 ${NC} ]"
cd ${TMP_DIR}
test ! -f lapack.tgz && \
    wget http://www.netlib.org/lapack/lapack.tgz
tar xzf lapack.tgz

echo -e "[ ${RED} Configure lapack-3.3.1 ${NC} ]"
cd lapack-3.3.1
sed -e 's/^OPTS *=/OPTS\t= -O2 -fPIC -m64/' \
    -e 's/^NOOPT *=/NOOPT\t=-O0 -fPIC -m64 /' \
    make.inc.example > make.inc

echo -e "[ ${RED} Build lapack-3.3.1 ${NC} ]"
cd SRC
make -j ${NPROCS}
...
../configure -b 64 -D c -DPentiumCPS=${CPUFREQ} \
    -Fa alg -fPIC \
    --with-netlib-lapack=../../lapack-3.3.1/lapack_LINUX.a -Si cputhrchk 0
